...
// no businesslogics
const Sample = ({ customDrinks, smaple, handleClick }: any) => (
  <div>
    {customDrinks.length &&
      customDrinks.map(
        ({ id, dateCreated, description, moodId, title, optionIds }: any) => (
          <div
            key={id}
          ...
          >
            ...
            <p>title: {title}</p>
            {optionIds.length &&
              optionIds.map((o: any) => <div key={o}>{o}</div>)}
          </div>
        ),
      )}
...
const SampleContainer = ({ customDrinks }: any) => {
